...
> **状态：历史实现基线，已被 [科学内核、错因闭环与 Dream 封版设计 v3](./科学内核与Dream封版v3/README.md) 替代。**
>
> v1 中的自研 I90、Dream 数值调整和 Dream 直接写错因状态不再是目标架构；保留本文仅用于追溯现有实现。
...
### 7.1 画像边界：单会话一切产出都不写画像

教学运行时（判答、单会话教学总结、**错因归因**、会话边界的选题判定）一律**不写长期画像**——
这是 ADR-004"教学模型无画像写权限"的同一纪律向错因/选题的延伸：

- 单会话 `DiagnosticClaim` 与 `TeachingSessionSummary` 是**事实层证据**，不是画像更新；
- 画像更新只发生在 Dream 路径（画像大模型综合双产物+错因证据后输出 PUD，Validator 校验后物化）。

### 7.2 错因两层（单会话归因 ≠ 画像错因状态）

| 层 | 动作 | 落点 |
|---|---|---|
| 单会话层 | 能归到已有 E-ID 就归已有；**归不上的标 `pending`（待定）** | 仅 DiagnosticClaim（事实层） |
| Dream 层（画像大模型） | ① 综合多会话错因证据更新画像错因六档状态；② **提取所有学生的待定错因，归纳新通用错因**（候选 E-ID → 教师复核后入错因库，走内容管线）；③ **学生特有且无法归类的待定错因 → 记入该生画像**（semantic_profile / misconceptions） | PUD → 快照物化 |

### 7.3 报告与计划都是画像子集

- **掌握度报告 = 画像的只读投影**（五档状态 + 证据展开 + 薄弱排序 + 复测到期），由画像系统直接提供，不是独立生成物；
- **学习计划 = 画像子集，但独立于 Dream 生成**：确定性约束排布 + 大模型转写（§10.3），在 pi 中实现（计划任务 skill），Dream 不触碰计划。

### 7.4 测评统一：聊天式测评 = 自适应测评

- 聊天式测评与 AssessmentRun（自适应测评）是**同一件事**：单会话内聊天推进（题目卡 → 作答 → 判答 → 错因追问卡 → …），
  **会话结束时由教学主模型判定下一步目标**（继续覆盖 / 错因消歧 / 前置下探 / 延迟复测 / 薄弱训练），
  传统程序（选题器）负责硬过滤与评分（§10 两阶段），下一会话的题目以卡片呈现；
- 自适应 = 传统程序（确定性）+ 教学主模型（会话边界语义判定）的结合，二者都不写画像。

### 7.5 教学卡片 = 通用教学原语（edu-agent 与卡片是同一设计）

- 卡片是教学环节**通用交互原语**：出题卡（题目）、错因追问卡（探针）、知识讲解卡（edu-agent HTML）、变式题卡；
  全部经统一 Artifact 协议（§5.4）与交互事件（card.presented / answer_submitted / skipped / free_text_requested）；
- edu-agent 生成的交互 HTML 与前端原生卡片是**同一设计**：渲染形态不同（sandboxed iframe vs 原生），
  提交同一 `ArtifactResponse` 事件，跳过/直接回复不产生失败观测（§1.1-10）；
- 自适应测评的"下一题"以卡片呈现：单会话内题目卡 → 作答 → 判答 →（错因追问卡）→ 会话结束 → 主模型判定 → 下一题卡。
