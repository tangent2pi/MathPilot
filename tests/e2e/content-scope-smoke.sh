...
POSTGRES_CONTAINER="${POSTGRES_CONTAINER:-mathpilot-dev-postgres-1}"

docker exec -i "$POSTGRES_CONTAINER" psql -U mathpilot -d mathpilot -v ON_ERROR_STOP=1 -P pager=off <<'SQL'
begin;
...
select mathpilot_provision_agent_identity('tnt_dev00001','content','usr_teacher01','scope-test-password');
select mathpilot_provision_agent_identity('tnt_dev00001','content','usr_scope_teacher02','scope-test-password');

set session authorization mathpilot_agent_content_tnt_dev00001_usr_teacher01;
do $$ begin
  if jsonb_array_length(mathpilot_agent_library('knowledge','K_SCOPE_SMOKE_PRIVATE',100,0)->'items') <> 1 then
    raise exception 'owner teacher did not see private content';
  end if;
end $$;
reset session authorization;

set session authorization mathpilot_agent_content_tnt_dev00001_usr_scope_teacher02;
do $$ begin
  if jsonb_array_length(mathpilot_agent_library('knowledge','K_SCOPE_SMOKE_PRIVATE',100,0)->'items') <> 0 then
    raise exception 'unrelated teacher saw private content';
  end if;
end $$;
reset session authorization;

set session authorization mathpilot_agent_tnt_dev00001_usr_student01;
do $$ begin
  if jsonb_array_length(mathpilot_agent_library('knowledge','K_SCOPE_SMOKE_PRIVATE',100,0)->'items') <> 0 then
    raise exception 'unbound student saw private knowledge';
  end if;
  if jsonb_array_length(mathpilot_agent_library('knowledge','K_SCOPE_SMOKE_PUBLIC',100,0)->'items') <> 1 then
