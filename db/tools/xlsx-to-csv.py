...
parser.add_argument("--xlsx", type=Path, default=ROOT / "data/knowledge-bank/解三角形/数学知识库_解三角形_完整版.xlsx")
parser.add_argument("--output", type=Path, default=ROOT / "data/official-content/triangle-kb")
parser.add_argument("--manifest", type=Path, default=ROOT / "db/migration-data/kb-triangle-manifest.csv")
args = parser.parse_args()
XLSX, OUT_DIR, MANIFEST = args.xlsx.resolve(), args.output.resolve(), args.manifest.resolve()
# Manifest paths must be repository-relative, as required by the importer.
OUTPUT_REL = OUT_DIR.relative_to(ROOT).as_posix()
