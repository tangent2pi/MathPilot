...
def emit() -> None:
    ...
    line("do $$")
    line("begin")
    line("  if not exists (select from pg_roles where rolname = 'agmath_app') then")
    line("    create role agmath_app login password 'agmath-app-dev-only';")
    line("  end if;")
    line("end $$;")
    line("grant usage on schema public to agmath_app;")
    line("grant select, insert, update, delete on all tables in schema public to agmath_app;")
    line("")
