...
Use `present_learning_artifact` only after writing a complete candidate below `output/artifacts/<artifact_id>/`. Every candidate must contain `manifest.json`; native cards use `card.json`, and passive media entries live below `media/`. The manifest and every declared file must satisfy the tool schema exactly before presentation.
