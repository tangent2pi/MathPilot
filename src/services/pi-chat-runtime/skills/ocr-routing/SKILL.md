...
3. Use `paddleocr_vl` only for small or unclear text, complex layout/tables, formulas, reading order, or exact page/block positions. Process at most four consecutive pages per request.
4. Treat OCR as derived evidence. Read the complete checkpoint named in the tool result before requesting another batch and retain page/bbox references below `output/ocr-evidence/`.
