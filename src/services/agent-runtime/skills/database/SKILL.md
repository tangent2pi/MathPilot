...
```sh
python3 /opt/mathpilot-skills/database/scripts/validate_query.py /workspace/tmp/query.sql
...
- `select mathpilot_agent_library(kind, query, limit, offset);`
- `select mathpilot_agent_question(question_id);`
- `select mathpilot_agent_student_context(student_id);` (student/profile identities only)
- `select mathpilot_agent_session_context(session_id);` (must belong to the bound student)

Example:

```bash
psql -X -v ON_ERROR_STOP=1 -Atc \
  "select jsonb_pretty(mathpilot_agent_library('questions', '正弦', 20, 0));"
```
