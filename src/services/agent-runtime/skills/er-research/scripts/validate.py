...
def main():
    ...
    if not isinstance(data,dict) or data.get("schema")!="agmath.er-result/v1": errors.append("schema must be agmath.er-result/v1")
    ...
    receipt.write_text(json.dumps({"schema":"agmath.validation-receipt/v1","skill":"er-research","result_file":result.name if result.parent.name=="output" else str(result),"sha256":digest,"valid":True,"error_cause_count":len(ecs),"rule_count":len(rules)},ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
