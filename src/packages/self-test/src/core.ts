...
export function replayDimension(observations: readonly DimensionObservation[], dimensionId: string) {
  ...
  for (const obs of observations) {
    if (obs.dimensionId !== dimensionId || obs.independent === false) continue;
    independentCount += 1;
    if (obs.correct) {
...
export function applyAnswerStep(
  ...
  event: { dimensionId: string; correct: boolean; format: string; independent?: boolean },
): RoundStepResult {
  const dim = (runtime.perDim[event.dimensionId] ??= {
    answered: 0, correct: 0, transfer: 0, lastFormat: null,
  });
  if (event.independent !== false) {
    dim.answered += 1;
