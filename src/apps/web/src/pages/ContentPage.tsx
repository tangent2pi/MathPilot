...
type Pipeline = {
  ...
  error_detail?: string;
  payload?: { files?: PipelineFile[]; publication?: { version?: string } };
};
...
function PipelineCard({ run }: { run: Pipeline }) {
  ...
  const add = useMutation({
    mutationFn: async (files: File[]) => {
      if ((run.document_ids?.length || 0) + files.length > 100) throw new Error("一个资料集最多保存 100 个文件。");
      if (files.some((file) => file.size > 20 * 1_048_576)) throw new Error("请移除超过 20 MiB 的文件后继续。");
  ...
  return <article className={`run-card ${run.status}`}>
    ...
    <div className="action-cluster">
      ...
      {run.status === "review_ready" && <Link className="btn cinnabar" to={`/teacher?view=review&queue=content&pipeline=${encodeURIComponent(run.run_id)}`}>{pending ? `继续复核（${pending}）` : "查看复核"}</Link>}
    </div>
    ...
    {run.status === "published" && run.payload?.publication && <p className="status-note success">版本 {run.payload.publication.version} 已发布</p>}
...
export function ContentPage() {
  ...
  const addFiles = (files: File[]) => setSelected((current) => { const seen = new Set(current.map(fileKey)); return [...current, ...files.filter((file) => !seen.has(fileKey(file)) && (seen.add(fileKey(file)) || true))]; });
  const invalid = selected.some((file) => file.size > 20 * 1_048_576) || selected.length > 100;
  ...
  return (
    <main className="page content-studio" id="main-content">
      <section className="page-hero compact studio-hero"><p className="eyebrow">内容</p><h1>整理一批新的教学资料</h1><p className="lede">添加讲义、题目或图片，检查清单后再开始处理。任务开始后，你可以随时回来查看进度和继续对话。</p></section>
      <div className="studio-grid">
        <section className="section-card upload-sheet"><div className="section-heading"><div><p className="eyebrow">新资料</p><h2>添加文件</h2></div><span id="selectionSummary" className="paper-tag">{selected.length} 个文件</span></div>
          ...
          <div id="fileList" className="file-list">{selected.length ? selected.map((file) => <article className="file-item" key={fileKey(file)}><div><strong>{file.name}</strong><small>{file.type || "待识别类型"} · {fileSize(file.size)}</small></div><button className="text-button" type="button" onClick={() => setSelected((current) => current.filter((item) => fileKey(item) !== fileKey(file)))}>移除</button></article>) : <EmptyState>添加本次要一起整理的资料</EmptyState>}</div>
          ...
          <p className={invalid || upload.isError ? "status-note error" : "muted"} aria-live="polite">{invalid ? selected.length > 100 ? "一个资料集最多选择 100 个文件。" : "请移除超过 20 MiB 的文件后继续。" : feedback}</p>
        </section>
